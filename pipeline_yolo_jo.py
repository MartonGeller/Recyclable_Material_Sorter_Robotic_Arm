...
model = YOLO("trashnet_yolov8n.pt")
class_map = ['glass','paper','cardboard','plastic','metal','trash']
...
while done != True: 
            ...
            while True:
                ...
                for o in out:
                    ...
                    for box in boxes:
                        ...
                        if box.conf[0] > 0.85 and (x2-x1 < 280 or y2-y1 < 280):
                            ...
                            cv2.putText(img, class_map[c], (x1,y1), font, 1, (255,0,0), 2)
                            if box.cls == 'plastic':
                                plastic_count += 1
                            elif box.cls == 'glass':
                                glass_count += 1
                                ...
                                cardboard_count += 1
                            time.sleep(0.3)
                            object_detected_event.set()  # Set the event when object is detected
    ...
    def pos(t):
        ...
        Q5 = Q4 + 7 * np.array([np.cos(t2+t3+t5)*np.cos(t1),np.cos(t2+t3+t5)*np.sin(t1), np.sin(t2+t3+t5)])
        UH = Q4 + 3.5 * np.array([np.cos(t2+t3+t5)*np.cos(t1),np.cos(t2+t3+t5)*np.sin(t1), np.sin(t2+t3+t5)])
    ...
    print(Q5)
    X = np.array([UH[0],UH[1],Q5[2]-3-mean]) # approximation of objects coordinates
    print(X)
